...
export async function POST(request: NextRequest) {
  try {
    ...
    const suggestion = await generateSuggestion(context, suggestionType);
...
function buildPrompt(context: CodeContext, suggestionType: string) {
...
4. Make the suggestion contextually appropriate`;
  ...
  const userPrompt = `Generate a ${suggestionType} suggestion.
  ...
  try {
    const response = await axios.post(
      "https://openrouter.ai/api/v1/chat/completions",
      {
        ...
        messages: buildPrompt(context, suggestionType),
        temperature: 0.7,
        max_tokens: 300,
      },
      {
